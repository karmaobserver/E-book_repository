...
import makso.rs.udd.indexer.handler.DocumentHandler;
...
public final class UDDIndexer {
	...
	public boolean updateDocument(Document doc, IndexableField... fields){
		//String ebookId = doc.get("ebookId");
	...
	public static DocumentHandler getHandler(File file){
		...
		//Posto nama treba samo PDFHandler
	...
	public void index(File folder, Ebook ebook) {
		// TODO Auto-generated method stub
		try {
			openIndexWriter();
			if(folder.isDirectory()){
				folderIndexer(folder,ebook);
			}else{
				//indeksiranje tacno jednog file-a
				fileIndexer(folder,ebook);
			}
			this.indexWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void fileIndexer(File file, Ebook ebook) {
		// TODO Auto-generated method stub
		DocumentHandler handler;
		handler = getHandler(file);
		if(handler != null){
			try {
				this.indexWriter.addDocument(handler.getDocument(file,ebook));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else{
	...
	private void folderIndexer(File folder, Ebook ebook) {
		// TODO Auto-generated method stub
		if(!folder.isDirectory()) return;
		File[] files = folder.listFiles();
		...
		for(File file : files){
			if(file.isFile()){
				fileIndexer(file,ebook);
			}else{
				folderIndexer(file,ebook);
			}
		}
	}
